...
diskCachePut' p res = do
    ...
    liftIO $ encodeFile metaPath (toResponseMeta res)
...
diskCacheGet' ::  (MonadIO m, MonadMask m) => FilePath -> m (Maybe (Response (BodyReaderM m)))
diskCacheGet' p = do
    mmeta <- liftIO $ decodeFileOrFail metaPath
    case mmeta of
        Left _ -> return Nothing
        -- TODO, this is fishy
        Right meta -> do hdl <- liftIO $ openFile dataPath ReadMode
...
-- | TODO: this doesn't check that request body is empty
...
-- Functions below are from `futurice-extra` 'System.IO.Fxtra'
